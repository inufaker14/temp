...
echo '■ WA-01. 관리서버 디렉터리 권한 설정'
echo '■ WA-01. 관리서버 디렉터리 권한 설정' >> $Apache_Result
...
for chk_WA_01 in `find $apache_root -type d`; do
  #echo $chk_WA_01

  # 디렉터리의 권한이 750보다 높으면 파일에 쓰기
  if [ `ls -dl $chk_WA_01 | grep "^.....-.---" | wc -l` -eq 0 ]; then
    ls -dl $chk_WA_01  >> $WA_01_TXT
...
echo '■ WA-02. 설정파일 권한 설정'
echo '■ WA-02. 설정파일 권한 설정' >> $Apache_Result
...
for chk_WA_02 in "${apache_conf_list[@]}"; do
  #echo $chk_WA_02

  # conf 파일의 권한 확인 후 일치하지 않으면 파일에 추가
  if [ `ls -l $chk_WA_02 | grep "^....------" | wc -l` -eq 0 ]; then
    ls -l $chk_WA_02  >> $WA_02_TXT
...
echo '■ WA-03. 디렉터리 검색 기능 제거'
echo '■ WA-03. 디렉터리 검색 기능 제거' >> $Apache_Result
...
if [ $filecheck_pass -eq 0 ]; then
  ...
  for chk_WA_03 in "${apache_conf_list[@]}"; do
    ...
    chkdata_WA_03=`cat $chk_WA_03 | egrep -i "<Directory |Indexes|</Directory" | grep -v '\#'`

    if [ `echo $chkdata_WA_03 | egrep -i "Indexes" | wc -l` -gt 0 ]; then
      #echo 'File name = '$chk_WA_03
      #echo $chkdata_WA_03

      echo 'File name = '$chk_WA_03 >> $WA_03_TXT
      echo "$chkdata_WA_03" >> $WA_03_TXT
      echo '' >> $WA_03_TXT
...
echo '■ WA-04. 로그 디렉터리/파일 권한 설정'
echo '■ WA-04. 로그 디렉터리/파일 권한 설정' >> $Apache_Result
...
if [ $filecheck_pass -eq 0 ]; then
  ...
  # apache의 로그 파일 조회
  for chk_WA_04 in `find $logs -type f -name *.log`; do
    #echo $chk_WA_04

    # conf 파일의 권한 확인 후 일치하지 않으면 파일에 추가
    if [ `ls -l $chk_WA_04 | grep "^...-.-----" | wc -l` -eq 0 ]; then
      ls -l $chk_WA_04  >> $WA_04_TXT
      WA_04_CHECK_COUNT=$((WA_04_CHECK_COUNT+1))
    fi
  done
...
echo '■ WA-05. 에러 메시지 관리'
echo '■ WA-05. 에러 메시지 관리' >> $Apache_Result
...
chkdata_WA_05=`cat "$conf" | egrep -i "ErrorDocument|400|401|403|404|500" | grep -v '\#'`
#echo 'File name = '$chk_WA_05
#echo "$chkdata_WA_05"

# 결과값의 문자열 길이 얻기
chkdatalen_WA_05=${#chkdata_WA_05}

# 문자열 길이가 0보다 크면 파일에 쓰기
if [ $chkdatalen_WA_05 -gt 0 ]; then
  echo 'File name = '$conf >> $WA_05_TXT
  echo "$chkdata_WA_05" >> $WA_05_TXT

  WA_05_CHECK_COUNT=$((WA_05_CHECK_COUNT+1))
fi

if [ $WA_05_CHECK_COUNT -eq 0 ]; then
...
echo '■ WA-06. 헤더 정보 노출 방지'
echo '■ WA-06. 헤더 정보 노출 방지' >> $Apache_Result
...
chkdata_WA_06=`cat $conf | grep -iEHn "^\s*ServerTokens|^\s*ServerSignature"`

# 결과값의 문자열 길이 얻기
chkdatalen_WA_06=${#chkdata_WA_06}
#echo "$chkdatalen_WA_06"

#echo "$chkdata_WA_06"
#echo "$chkdata_WA_06" | wc -l
...
if [ `cat $WA_06_TXT | grep -i "ServerTokens" | grep -i "Prod" | wc -l` -eq 0 ]; then
  ...
  if [ `cat $WA_06_TXT | grep -i "ServerSignature" | grep -i "Off" | wc -l` -eq 0 ]; then
    echo 'WA-06,X,,' >> $Apache_Result
...
echo '■ WA-07. FollowSymLinks 옵션 비활성화'
echo '■ WA-07. FollowSymLinks 옵션 비활성화' >> $Apache_Result
...
if [ $filecheck_pass -eq 0 ]; then
  # apache의 conf 파일 조회
  for chk_WA_07 in "${apache_conf_list[@]}"; do
    #echo $chk_WA_07

    # conf 파일의 FollowSymLinks를 검색
    chkdata_WA_07=`cat $chk_WA_07 | egrep -i "<Directory |FollowSymLinks|</Directory" | grep -v '\#'`

    if [ `echo $chkdata_WA_07 | egrep -i "FollowSymLinks" | wc -l` -gt 0 ]; then
      #echo 'File name = '$chk_WA_07
      #echo $chkdata_WA_07

      echo 'File name = '$chk_WA_07 >> $WA_07_TXT
  ...
  if [ $WA_07_CHECK_COUNT -gt 0 ]; then
...
echo '■ WA-08. MultiView 옵션 비활성화'
echo '■ WA-08. MultiView 옵션 비활성화' >> $Apache_Result
...
if [ $filecheck_pass -eq 0 ]; then
  ...
  for chk_WA_08 in "${apache_conf_list[@]}"; do
    ...
    chkdata_WA_08=`cat $chk_WA_08 | egrep -i "<Directory |MultiViews|</Directory" | grep -v '\#'`

    if [ `echo $chkdata_WA_08 | egrep -i "MultiViews" | wc -l` -gt 0 ]; then
...
echo '■ WA-09. HTTP Method 제한'
echo '■ WA-09. HTTP Method 제한' >> $Apache_Result
...
echo '■ WA-10. 로그 포맷 설정'
echo '■ WA-10. 로그 포맷 설정' >> $Apache_Result
...
echo "WA-11 Check Start..."
echo "■ WA-11. 웹서비스 웹 프로세스 권한 제한" >> $Apache_Result 2>&1
...
if [ `ps -ef | grep "httpd" | grep -v "ns-httpd" | grep -v "grep" | awk '{print $8}' | grep "/" | grep -v "httpd.conf" | uniq | wc -l` -gt 0 ]; then
	...
	if [ `cat $ACONF | grep -i "user" | grep -v "\#" | egrep -v "^LoadModule|LogFormat|IfModule|UserDir" | grep -i "user" | grep -i "root" | wc -l` -gt 0 ]; then
		echo "WA-11,X,,"	>> $Apache_Result 2>&1
		...
		if [ `cat $ACONF | grep -i "user" | grep -v "\#" | egrep -v "^LoadModule|LogFormat|IfModule|UserDir" | grep -i "group" | grep -i "root" | wc -l` -gt 0 ]; then
			...
			echo "WA-11,O,,"	>> $Apache_Result 2>&1
...
echo "■ WA-12. 웹서비스 상위 디렉터리 접근 금지" >> $Apache_Result 2>&1
...
echo "■ WA-13. 웹서비스 파일 업로드 및 다운로드 제한" >> $Apache_Result 2>&1
...
if [ `ps -ef | grep "httpd" | grep -v "ns-httpd" | grep -v "grep" | awk '{print $8}' | grep "/" | grep -v "httpd.conf" | uniq | wc -l` -gt 0 ]; then
	...
	if [ `cat $ACONF | egrep -i "<Directory |LimitRequestBody|</Directory" | grep -v '\#' | wc -l` -eq 0 ]; then
		...
		echo "WA-13,C,,"	>> $Apache_Result 2>&1
...
echo "■ WA-14. 웹 서비스 영역의 분리" >> $Apache_Result 2>&1
...
echo "■ WA-15. 3. 서비스 관리 > 3.20 웹서비스 불필요한 파일 제거" >> $Apache_Result 2>&1
